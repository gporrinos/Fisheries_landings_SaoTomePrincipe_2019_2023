...
catchcompositionplots <- function(gearlist, type, threshold = 5, taxonomical_level = "Family", 
  ...
  for(i in 1:length(sp)){
    if(paste0(sp[i],".JPG") %in% list.files(paste0(figures,"/fishoutlines"))) {
      fish = jpeg::readJPEG(source = paste0(figures,"/fishoutlines/",sp[i],".JPG"),native=TRUE)
    } else {
      if(paste0(sp[i],".jpg") %in% list.files(paste0(figures,"/fishoutlines"))) {
        fish = jpeg::readJPEG(source = paste0(figures,"/fishoutlines/",sp[i],".jpg"),native=TRUE)
      } else {
        fish = jpeg::readJPEG(source = paste0(figures,"/fishoutlines/blank.JPG"),native=TRUE)
      }}
